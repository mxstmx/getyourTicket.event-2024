                    ...
                    <div class="col-lg-4 col-md-12 col-12">
                        <div class="card bg-info mb-4 border-0 shadow-sm">
                            <div class="card-body p-4">
                                ...
                                <div class="d-flex align-items-center justify-content-between" v-if="total_earnings.length <= 0">
                                    <h2 class="fw-bold mb-0 fs-3 text-white">0</h2>
                                    <i class="fas fa-cart-arrow-down text-white fs-5"></i>
                                </div>

                                <div class="d-flex align-items-center justify-content-between mb-2 border-bottom" v-else v-for="(total_earning, index) in total_earnings" :key="index">
                                    <h2 class="fw-bold mb-0 fs-3 text-white">{{ total_earning.customer_paid_total }} {{ total_earning.currency }}</h2>
                                    <i class="fas fa-cart-arrow-down text-white fs-5"></i>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-4 col-md-12 col-12">
                        <div class="card bg-dark mb-4 border-0 shadow-sm">
                            <div class="card-body p-4">
                                ...
                                <div v-else class="d-flex align-items-center justify-content-between mb-2 border-bottom" v-for="(total_earning, index) in total_earnings" :key="index">
                                    <h2 class="fw-bold mb-0 fs-3 text-white">{{ total_earning.admin_commission_total }} {{ total_earning.currency }}</h2>
                                    <i class="fas fa-user-shield text-white fs-5"></i>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-4 col-md-12 col-12">
                        <div class="card bg-success mb-4 border-0 shadow-sm">
                            <div class="card-body p-4">
                                ...
                                <div class="d-flex align-items-center justify-content-between" v-if="total_earnings.length <= 0">
                                    <h2 class="fw-bold mb-0 fs-3 text-white">0</h2>
                                    <i class="fas fa-sack-dollar text-white fs-5"></i>
                                </div>
                                <div v-else class="d-flex align-items-center justify-content-between mb-2 border-bottom" v-for="(total_earning, index) in total_earnings" :key="index">
                                    <h2 class="fw-bold mb-0 fs-3 text-white">{{ total_earning.organiser_earning_total }} {{ total_earning.currency }}</h2>
                                    <i class="fas fa-sack-dollar text-white fs-5"></i>
                                </div>
                            </div>
                        </div>
                    ...
                    <tbody>
                        <tr v-for="(earning, index) in event_earning" :key="index" >
                            <td :data-title="trans('em.event')">
                                <div class="d-flex align-items-center justify-content-start gap-3">
                                    ...
                                    <div class="ms-3 lh-1 w-50">
                                        <h5 class="mb-1"> 
                                            <a :href="eventSlug(earning.event_slug)" class="text-inherit text-wrap h4">{{ earning.event_name }}</a>
                                        </h5>
                                        <small class="text-success strong"><i class="fas fa-bolt"></i> {{ earning.count_bookings }} {{ trans('em.bookings') }}</small>
                                    </div>
                                    <div class="card-base d-flex flex-column bg-info text-gray-700 w-fit">
                                        <h4 class="m-0 p-0 fw-semibold mb-1">
                                            {{ moment(earning.month_year, 'MM YYYY').format('MMM ,YYYY') }}
                                        </h4>
                                        <div class="d-flex gap-3">
                                            <div class="me-2">{{ trans('em.bookings') }}:</div>
                                            <div class="text-end">{{ earning.customer_paid_total }} {{ earning.currency }}</div>
                                        </div>
                                        <div class="d-flex gap-3">
                                            <div class="me-2">{{ trans('em.commission') }}:</div>
                                            <div class="text-end">{{ earning.admin_commission_total }} {{ earning.currency }}</div>
                                        </div>
                                        <div class="d-flex gap-3">
                                            <div class="me-2">{{ trans('em.profit') }}:</div>
                                            <div class="text-end"><i class="fas fa-sack-dollar"></i> {{ earning.organiser_earning_total }} {{ earning.currency }}</div>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <!-- <td class="align-middle" :data-title="trans('em.bookings')">
                                {{ earning.customer_paid_total }} {{ earning.currency }}
                            </td> -->
                            <!-- <td class="align-middle" :data-title="trans('em.commission')">
                                {{ earning.admin_commission_total }} {{ earning.currency }}
                            </td> -->
                       
                            <!-- <td class="align-middle text-success" :data-title="trans('em.profit')">
                                <i class="fas fa-sack-dollar"></i> {{ earning.organiser_earning_total }} {{ earning.currency }}
                            </td> -->
                            <!-- <td class="align-middle" :data-title="trans('em.month')">
                                {{ moment(earning.month_year, 'MM YYYY').format('MMM ,YYYY') }}
                            </td> -->
                            <td class="align-middle" :data-title="trans('em.transferred')">
                                <span class="btn btn-dash-success px-3 text-success" v-if="earning.transferred > 0">{{ trans('em.paid') }}</span>
                                <span class="btn btn-dash-primary px-3 text-primary" v-else>{{ trans('em.pending') }}</span>
                            </td>
                        </tr>
    ...
    methods:{
        ...
        totalEarning() {
            ...
            .then(res => {
                ...
                this.total_earnings   = res.data.total_earning;
