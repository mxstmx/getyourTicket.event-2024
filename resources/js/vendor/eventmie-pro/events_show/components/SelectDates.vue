...
<div class="col-xs-12">
    ...
    <div v-else >
        ...
        <div v-if="event.repetitive > 0">
            ...
            <div>
                <div class="d-flex  bg-white tab-card gap-2 w-100 justify-content-start mb-3 h-scroll">
                    <div
                        ...
                        :class="'mb-1'"
                    >
                        <a class="pointer py-1 px-3 chart-color4 sub-text fw-medium tab"
                            :class="{'active' : index == tab_active_index}" v-if="moment(moment(local_from_date[index], 'YYYY-MM-DD').format('MM  YYYY'), 'MM  YYYY').isAfter(moment(moment().format('MM  YYYY'), 'MM  YYYY')) || moment(moment(local_from_date[index], 'YYYY-MM-DD').format('MM  YYYY'), 'MM  YYYY').isSame(moment(moment().format('MM  YYYY'), 'MM  YYYY'))"
                        >
            ...
            <div class="tab-content v-scroll mh-30 p-lg-2 p-1 chart-color1 rounded-4" v-if="event.merge_schedule <= 0">
                        ...
                        >
                            ...
                            >
                                ...
                                @click="
                                            ...
                                            : null">
                                    <span>{{ userTimezone1(dateToShortDate(selected_date+' '+schedules[index].from_time, schedules[index].from_date), date_format.vue_date_format).format('DD') }}</span>

                                    <!-- expired -->
                                    <span class="mb-0 h6 badge bg-danger event-ended" v-if="userTimezone1(item+'-'+selected_date+' '+schedules[index].to_time,'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')"
                                    >
                                        <span>{{ trans('em.ended') }}</span>
                                    </span>

                                    <!-- out-of-stock -->
                                    <span class="mb-0 h6 badge bg-danger event-outofstock" v-if="
                                        !(userTimezone1(item+'-'+selected_date+' '+schedules[index].to_time,'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')) && !available_dates[index][index1]"
                                    >
                                    ...
                                    <!-- filling fast -->
                                    <span class="mb-0 h6 badge bg-danger event-outofstock mr-3" v-if="
                        ...
                        >
                            ...
                            >
                                ...
                                @click="
                                            ...
                                            : null">
                                    <span class="mb-0 fw-bold">{{ userTimezone(dateToShortDate(selected_date+' '+schedules[index].from_time, schedules[index].from_date), date_format.vue_date_format).format('DD') }}</span>

                                    <!-- expired -->
                                    <span class="mb-0 h6 badge bg-danger" v-if="userTimezone(item+'-'+selected_date+' '+schedules[index].to_time,'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')"
                                    >
                                        <span>{{ trans('em.ended') }}</span>
                                    </span>

                                    <!-- out-of-stock -->
                                    <span class="mb-0 h6 badge bg-danger" v-if="
                                        !(userTimezone(item+'-'+selected_date+' '+schedules[index].to_time,'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')) && !available_dates[index][index1]"
                                    >
                                        <span>{{ trans('em.out_of_stock') }}  </span>
                                    </span>
                                    ...
                                    <!-- filling fast -->
                                    <span class="mb-0 h6 mr-3 badge bg-warning" v-if="
            ...
            <div class="tab-content v-scroll mh-30 p-lg-2 p-1 chart-color1 rounded-4" v-if="event.merge_schedule > 0 && parseInt(schedules[0].repetitive_type) == parseInt(3)">
            ...
            <div class="tab-content v-scroll mh-30 p-lg-2 p-1 chart-color1 rounded-4" v-if="event.merge_schedule > 0 && parseInt(schedules[0].repetitive_type) == parseInt(2)">
                <div v-for="(item, index) in calculate_months" :key="index" class="tab-pane" :class="{'active' : index == tab_active_index}"
                    v-if="schedules[index].from_time != null && schedules[index].to_time != null "
                >
                    <div class="row" role="tablist" aria-multiselectable="true">
                        ...
                        >
                            ...
                            >
                                <div class="date-tab-content">
                                    <!-- <span class="mb-0 span fw-bold">{{changeTimeFormat(userTimezone(moment(schedules[index].from_date).format('YYYY-MM')+'-'+selected_dates[index]+' '+schedules[index].from_time, 'YYYY-MM-DD HH:mm:ss').format('HH:mm:ss')) }} - {{ changeTimeFormat(userTimezone(moment(schedules[index].to_date).format('YYYY-MM')+'-'+selected_dates[index]+' '+schedules[index].to_time, 'YYYY-MM-DD HH:mm:ss').format('HH:mm:ss'))}}</span> -->

                                <!-- expired -->
                                    <span class="mb-0 badge bg-danger" v-if="moment(moment(getWeekLastDate(item2, selected_dates[index], schedules[index].from_date)).format('YYYY-MM-DD')+' '+schedules[index].to_time,'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss') < moment().format('YYYY-MM-DD HH:mm:ss')"
                                    >
                                        <span>{{ trans('em.ended') }} </span>
                                    </span>

                                    <!-- out-of-stock -->
                                    <span class="mb-0 badge bg-danger" v-if="
                                    ...
                                    <!-- filling fast -->
                                    <span class="mb-0 badge bg-warning mr-3" v-if="
                                    ...
                                    <div class="d-flex w-100 justify-content-center">
                                        ...
                                        >
                                            {{ userTimezone(dateToShortDate(selected_date+' '+schedules[index].from_time, schedules[index].from_date ),date_format.vue_date_format).format('DD') }}
                                        </span>
                                    </div>
        ...
        <div class="lgx-tab" v-if="event.repetitive <= 0">
            <div class="tab-content v-scroll mh-30 p-lg-2 p-1 chart-color1 rounded-4">
                <div class="tab-pane active">
                    <div class="row" role="tablist" aria-multiselectable="true">
                        <a role="button" data-toggle="collapse" aria-expanded="true" aria-controls="collapseOne">
                            <div class="card border-0 shadow-sm img-hover mb-2 lgx-single-schedule"
