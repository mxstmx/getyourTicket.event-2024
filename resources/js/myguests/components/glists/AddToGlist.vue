<template>
        <div class="custom_model">
            ...
            <div class="modal show" v-if="g_event_id > 0">
                <div class="modal-dialog modal-lg w-100">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5"> {{ trans('em.add_attendees_to_guestlist') }}</h1>
                        ...
                        <div class="modal-body">
                            <form ref="form" @submit.prevent="validateForm" method="POST" enctype="multipart/form-data">
                                <input type="hidden" class="form-control " v-model="glist_ids" name="glist_ids"
                                >
                                <input type="hidden" class="form-control " v-model="organiser_id
                                " name="organiser_id">
                                
                                <input type="hidden" class="form-control " v-model="g_event_id" name="event_id"
                                >
                                <div class="modal-body">
                                    <div class="mb-3">
                                        <label class="form-label">{{ trans('em.select') }} {{ trans('em.guestlist') }}</label>
                                        <multiselect
                                ...
                                <div class="modal-footer">
                                    <button type="submit" class="btn btn-primary"><i class="fas fa-sd-card"></i> {{ trans('em.add')+' '+trans('em.to')+' '+trans('em.guestlist') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
