        ...
        @if (!empty(json_decode($venue->images, true)))
            {{-- <div class="container-fluid p-0">
                <div class="cover-img-bg"
                    style="background-image: url({{ '/storage/' . json_decode($venue->images, true)[0] }});">
                    <img class="cover-img" src="{{ '/storage/' . json_decode($venue->images, true)[0] }}"
                        alt="{{ $venue->title }}" />
                </div>
            </div> --}}
            <div class="container mt-lg-12 mt-10">
                <div class="rounded-5 bg-white position-relative p-lg-4 p-3">
                    <div class="row">
                        <div class="col-xl-12 col-lg-12 col-md-12 col-12">
                            <img src="{{ '/storage/' . json_decode($venue->images, true)[0] }}" alt="{{ $venue->title }}"
                                class="img-fluid rounded-4 w-100 img-cover h-auto">
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <div class="zoom-gallery">
                                    <div class="row">
                                        @foreach (json_decode($venue->images, true) as $val)
                                            <div class="col-md-6">
                                                <img src="{{ '/storage/' . $val }}" class="img-fluid rounded mb-2"
                                                    alt="{{ $venue->title }}" />
                                            </div>
                                        @endforeach
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <p class="mb-0">
                                            ...
                                            @if ($venue->zipcode)
                                                {{ ', ' . $venue->zipcode }}
                                            @endif
                                            @if (!empty($venue->country['country_name']))
                                                {{ ', ' . $venue->country['country_name'] }}
                                            @endif
                                        </p>
                                    </div>
                                </div>
                                        ...
                                        <form method="POST" class="lgx-contactform"
                                            ...
                                            <div class="form-group mb-3">
                                                <input type="text" name="guests" class="form-control lgxsubject"
                                                    id="lgxsubject"
                                                    placeholder="{{ __('eventmie-pro::em.number_of_guests') }}"
                                                    aria-required="true">
                                            </div>
                                            <div class="form-group mb-3">
...
@section('javascript')
    <script
        src="https://maps.googleapis.com/maps/api/js?key={{ setting('apps.google_map_key') }}&callback=initMap&v=weekly"
        defer></script>
    <script>
        function initMap() {
            const myLatLng = {
