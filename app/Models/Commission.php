...
class Commission extends Model
{
    ...
    public function admin_commission()
    {
        ...
        $result = $query->select($select)
                        ...
                        ->join('bookings', 'commissions.booking_id', '=', 'bookings.id')->groupBy('bookings.currency')
                        ->groupBy("commissions.organiser_id", "bookings.currency")
                        ->get();
    ...
    public function show_commission_organisers_wise($organiser_id = null, $params = [], $refunds = false)
    {
        ...
        $query->join('bookings', 'commissions.booking_id', '=', 'bookings.id')->groupBy('bookings.currency');
    ...
    public function admin_edit_commission($params)
    {
        ...
        return Commission::where([
            'commissions.organiser_id'  => $params['organiser_id'], 
            'commissions.month_year'    => $params['month_year'], 
            'commissions.event_id'      => $params['event_id'],
            // change transferred status of only enabled commissions
            'commissions.status'        => 1,
            'bookings.currency'         => request()->currency
