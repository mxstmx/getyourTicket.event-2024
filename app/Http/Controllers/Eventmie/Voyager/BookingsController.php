...
use App\Models\Booking as BaseBooking;

use App\Http\Controllers\Eventmie\SendEmailController;
...
class BookingsController extends BaseBookingsController
{
     ...
     * @return void
     */
    public function send_email(BaseBooking $booking, Request $request)
    {
        $booking_data = BaseBooking::where(['common_order' => $booking->common_order])->get();
        session(['booking_email_data'=> $booking_data->toArray()]);

        $sendEmail    =  new SendEmailController;
        
        $sendEmail->send_email($request);

        $msg = __('eventmie-pro::em.booking_email_sent');
