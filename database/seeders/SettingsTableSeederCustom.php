...
class SettingsTableSeederCustom extends Seeder
{
    ...
    public function run()
    {
        ...
        $setting = $this->findSetting("apps.stripe_webhook_secret_key");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Stripe Webhook Secret Key", "value" => "", "details"=> null, "type" => "text", "order" => "51", "group" => "Apps", ])->save();
        }
